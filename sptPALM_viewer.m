...
SimulationParameters.Offset = 99; % intensity offset for the image
...
%% ==========================================================
    ...
    function Change_AquisitionParameters(~,~)
        ...
        if ~isempty(NewAquisitionParameters)
            ...
            h.SimulationParameters.QY = str2double(NewAquisitionParameters{5}); % quantum yield of the detector
            h.SimulationParameters.CCDsensitivity = str2double(NewAquisitionParameters{6}); % CCD sensitivity (conversion electrons to AD counts)
            h.SimulationParameters.ReadoutNoise = str2double(NewAquisitionParameters{7}); % readout noise of the detector
            h.SimulationParameters.GaussStampSize = str2double(NewAquisitionParameters{8}); % in um
            h.SimulationParameters.LimitResolution = str2double(NewAquisitionParameters{9}); % Mean intensity of single emitters
        end
    end
