...
for nFrame = 1 : NFrames
    ...
    for nProt = 1 : N
        ...
        % average diffusion coefficient. In order to get a "realistic" 
        % simulation of the motion, each iteration (that is, each step 
        % separating two successive images) is subdivised into 1000 sub-steps.
        % For each sub-step, the distance travelled by the particle is the
        % same and defined by the diffusion coefficient. However, the 
        % direction of the movement is completely random. In the end, the 
        % position at the end of each step is the consequence of the 1000
        % "collisions" and is a more realistic way to simulate the motion
        % of the particle.
        % ----------------
...
%             Traj(dt+1,:) = [nFrame+dt, Traj(dt,2)+d*cosd(Theta), Traj(dt,3)+d*sind(Theta), d];
            ...
            Nsubstep = 5000;
            Sub_AcquisitionTime = AcquisitionTime/Nsubstep;
            d = sqrt(4*Diff*Sub_AcquisitionTime)/PixelSize;
            Theta = random('Uniform', 0, 360, [Nsubstep,1]);
            Traj(dt+1,:) = [nFrame+dt, Traj(dt,2)+d*sum(cosd(Theta)), Traj(dt,3)+d*sum(sind(Theta)), sqrt( (d*sum(cosd(Theta)))^2 + (d*sum(sind(Theta)))^2 )];
