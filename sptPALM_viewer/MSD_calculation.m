...
for ntraj = 1 : NTraj_ROI
    ...
    Dx = max(Traj(2,:))-min(Traj(2,:));
    Dy = max(Traj(3,:))-min(Traj(3,:));
    
    % Sometimes, TrackMate returns an error for the position of object (all
    % positions are equal to zero), which in turns creates an error for the
    % calculation of D. To avoid this issue, a first check is performed on
    % the coordinates.
    % ----------------
    ...
    if Dx>0 && Dy>0
        ...
        % Also sometimes the weights values is diverging to Inf. When this
        % issue happens, the associated trajectories are not taken into
        % account.
        % ---------
        
        if size(MSD,2)>=p && isempty(find(Weight==Inf, 1))
            MSD_all{ntraj} = MSD;
